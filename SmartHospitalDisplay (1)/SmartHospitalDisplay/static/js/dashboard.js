...
class HospitalDashboard {
    ...
    displayAlerts(alerts) {
        ...
        // Update active alerts count in header
        const alertsCountElement = document.getElementById('activeAlertsCount');
    ...
    async loadTokenSummary() {
        try {
            console.log('Loading token summary...');
            const response = await fetch('/api/tokens');
            console.log('Token summary response status:', response.status);
            ...
            const data = await response.json();
            console.log('Token summary data:', data);
    ...
    displayTokenSummary(data) {
        ...
        const totalPatients = Object.values(queue).reduce((total, dept) => total + dept.length, 0);
        
        // Update patients count in header
        const patientsCountElement = document.getElementById('totalPatients');
                ...
                <div class="row mb-3">
                    <div class="col-md-6">
                        <div class="d-flex align-items-center">
                            <span class="department-badge me-2">${this.formatDepartmentName(dept)}</span>
                        </div>
                    </div>
                    <div class="col-md-3">
    ...
    formatDepartmentName(dept) {
        ...
        return departmentNames[dept] || dept.charAt(0).toUpperCase() + dept.slice(1);
    }
